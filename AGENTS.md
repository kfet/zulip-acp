...
`make all` and then GoReleaser, which publishes the GitHub release and updates
`Formula/zulip-acp.rb` in the shared `kfet/homebrew-ai` tap. Users install with
`brew install kfet/ai/zulip-acp`.

`THIRD_PARTY_NOTICES.md` is regenerated by `make all` — i.e. BEFORE the release
commit is created — so the tagged tree and the uploaded asset carry the notices
for the dependency set actually being released. `make publish` never commits it:
by the time publish runs, the tag exists, and anything committed then falls
outside the tagged tree. Publish instead refuses to push when the notices are
stale (or when the tag does not point at `HEAD`) and tells you to amend and
re-tag.
