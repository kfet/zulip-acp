...
- [ ] Instance has a spec in `poe-acp/bots/` and `dist.lock` `.relays["zulip-acp"]` pins its version.
- [ ] **`poe-acp/scripts/converge.sh status` run, and the `zulip-zbox` row is `ok`.**
