...
- **The per-conversation cwd and staged attachments are created on THIS host,
  ...
  errors anywhere. The primitive is already shared: `acp-kit/remotefs`
  (`Mkdir` / `Push` / `Fetch`, ssh+tar, BatchMode, bounded, `Local` = no-op).
  Adopt the same operator key, `agent_ssh_host`, provision before session
  ACQUISITION (load and resume carry the cwd too, not just new), and fail
  session creation loudly rather than falling through to a wrong cwd.
